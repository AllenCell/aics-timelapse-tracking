...
def img_to_nodes(im_shape: np.ndarray, fov: List[Dict], meta: Optional[Dict] = None) -> List[Dict]:
    ...
    for index in fov.index:
        ...
        for idx_d, dim in enumerate("zyx"[-len(field_shape) :]):
            ...
            if dim != "z":
                edge_distance = np.amin(
                    [edge_distance, rcm[idx_d], field_shape[idx_d] - rcm[idx_d]]
                )
            ...
            for idx_d, dim in enumerate("zyx"[-len(im_shape) :]):
                ...
                if dim != "z":
                    edge_distance = np.amin(
                        [
                            edge_distance,
                            pair_node[f"centroid_{dim}"],
                            field_shape[idx_d] - pair_node[f"centroid_{dim}"],
                        ]
                    )
...
def _img_to_nodes_wrapper(in_args) -> List[Dict]:
    ...
    print(meta["path_tif"])
    fov = pd.read_csv(
        meta["path_tif"].replace(".tiff", ".tif").replace(".tif", "_region_props.csv")
    )  # , index_col=0)
...
def dir_to_nodes(
    ...
    df = (
        pd.concat([df[[not i for i in df.is_pair.values]], df[df.is_pair.values]])
