...
def test__track_overlap():
    ...
    overlap = metrics._track_overlap(df_a=df_a, df_b=df_b, thresh_dist=32.)
    assert overlap == 6  # 6 / 8 nodes covered

    # Threshold set to 0, so no nodes should overlap.
    overlap = metrics._track_overlap(df_a=df_a, df_b=df_b, thresh_dist=0.)
...
def test_target_effectiveness():
    ...
    coords_b = np.array([-10.*i for i in range(20, 24 + 1)])
    df_one_track = (
        pd.DataFrame(
    ...
    df_tracks = (
        pd.concat([df_two_tracks, df_one_track])
