...
def _gen_dummy_image_dir(
    ...
    for idx, centroids in enumerate(centroid_lists):
        img = _gen_dummy_image(centroids=centroids, shape=shape, first_label=first_label)
...
def test_img_to_nodes_v2(centroids: Sequence[Sequence[int]], shape: Sequence[int]):
...
def test_dir_to_nodes(tmp_path):
    ...
    centroid_z_exp = [3.0, 80.0, 30.0, 64.0]
    centroid_y_exp = [4.0, 80.0, 60.0, 128.0]
    centroid_x_exp = [5.0, 80.0, 120.0, 256.0]
