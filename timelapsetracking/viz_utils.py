...
def get_color_mapping(identifiers: Sequence[int], cmap_mpl: str = "hsv") -> Dict[int, Color]:
...
def _overlay(img_b: np.ndarray, img_o: np.ndarray) -> np.ndarray:
    ...
    if not np.issubdtype(img_b.dtype, np.unsignedinteger) or not np.issubdtype(
        img_o.dtype, np.unsignedinteger
    ):
...
def _blend_lighten_only(img_a: np.ndarray, img_b: np.ndarray) -> np.ndarray:
    ...
    if not np.issubdtype(img_a.dtype, np.unsignedinteger) or not np.issubdtype(
        img_b.dtype, np.unsignedinteger
    ):
        raise TypeError("Images must be np.unsignedinteger compatible")
...
def visualize_objects(
    ...
    ar = np.frombuffer(fig.canvas.tostring_rgb(), dtype=np.uint8).reshape(shape_fig)
    plt.close(fig)
    return ar
...
def timelapse_3d_to_2d(path_in_dir: Path, path_out_dir: Path, val_object: int = 64) -> None:
    ...
    for idx_t, path_tif in enumerate(paths_tifs):
        ...
        img[img_line] = 255
        path_save = Path(path_out_dir, f"{path_in_dir.name}.{idx_t:03d}.z{z_index}.tif")
...
def visualize_tracks_2d(
    ...
    centroids = df.apply(lambda x: np.array([x["centroid_y"], x["centroid_x"]]), axis=1)
    ...
    for idx, df_g in df.groupby("index_sequence"):
        ...
        for _, row in df_g.iterrows():
            indices_segs = [
                int(x) for x in row.in_list[1:-1].replace(" ", "").split(",") if x.isdigit()
            ]
        ...
        labels = [
            f"{int(l)}:{int(t)}"
            for l, t in zip(df_g["lineage_id"].tolist(), df_g["track_id"].tolist())
        ]
