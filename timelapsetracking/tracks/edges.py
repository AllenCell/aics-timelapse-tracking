...
def add_edges(
    ...
    **kwargs
    ...
    df_edges = pd.DataFrame(index=df.index, columns=["in_list", "out_list"]).fillna("[]")
    ...
    for idx_s in tqdm(range(index_vals.min() + 1, index_vals.max() + 1)):
        ...
        keep_idxs = [index for index, row in df_prev.iterrows() if not row["is_pair"]]
        df_prev = df_prev.loc[keep_idxs].copy()

        df_curr = df.query(f"{col_index_sequence} == @idx_s")

        is_pair = [idx for idx, row in df_curr.iterrows() if row["is_pair"]]
        ...
        keep_idxs = [index for index, row in df_curr_has_pair.iterrows() if row["has_pair"]]
        ...
        for edge_from, edge_to in pair_edges:
            ...
            if df_edges.loc[idxs_curr, "is_pair"].values:
                ...
                for label in df_edges.loc[idxs_curr, "label_img"].values[0]:
                    child_list.append(
                        df_curr_has_pair.index[df_curr_has_pair["label_img"] == label].values[0]
                    )
        ...
        if is_pair is not None:
            matched_nodes = [
                df_curr.index[edge_to]
                for edge_from, edge_to in edges
                if not ((edge_from is None) or (edge_to is None))
            ]
    ...
    # attempt to match detected  mitotic pairs with no parent cell
...
def add_pseudo_pairs(
    df_edges: pd.DataFrame, pairs_to_redo, col_index_sequence: str = "index_sequence", **kwargs
) -> pd.DataFrame:
    """Attempt to find previous instances of a pair of daughter cell pairs based
    on backtracking from a successful detection in a later frame.
    ...
    **kwargs
    ...
    for pair_idx in tqdm(pairs_to_redo):
        ...
        pseudo_child_1, pseudo_child_2 = find_psuedo_pair(
            child_1, child_2, df_edges, col_index_sequence, start=True
        )
        ...
        if pseudo_child_1 is not None or pseudo_child_2 is not None:
            ...
            pseudo_pair = {}
            pseudo_pair["volume"] = (
                pseudo_child_1["volume"].values[0] + pseudo_child_2["volume"].values[0]
            )
            ...
            pseudo_pair["edge_cell"] = bool(
                pseudo_child_1["edge_cell"].values[0] + pseudo_child_2["edge_cell"].values[0]
            )

            for _, dim in enumerate("zyx"):
                pseudo_pair[f"centroid_{dim}"] = (
                    pseudo_child_1[f"centroid_{dim}"].values[0]
                    + pseudo_child_2[f"centroid_{dim}"].values[0]
                ) / 2

            pseudo_pair["edge_distance"] = (
                pseudo_child_1["edge_distance"].values[0]
                + pseudo_child_2["edge_distance"].values[0]
            ) / 2
            ...
            pseudo_pair["volume"] = (
                (pseudo_child_1["volume"].values[0] + pseudo_child_2["volume"].values[0])
                + (pseudo_child_1["volume"].values[0] + pseudo_child_2["volume"].values[0]) / 2
            ) / 2
...
def find_psuedo_pair(child_1, child_2, df_edges, col_index_sequence, start=False):
    ...
    # otherwise recursively search further back
        ...
        pseudo_child_1, pseudo_child_2 = find_psuedo_pair(
            new_child_1, new_child_2, df_edges, col_index_sequence
        )
        return pseudo_child_1, pseudo_child_2


def bridge_edges(
    df_edges: pd.DataFrame, col_index_sequence: str = "index_sequence", window: int = 3, **kwargs
) -> pd.DataFrame:
    ...
    **kwargs
    ...
    for idx_s in tqdm(range(index_vals.min() + 1, index_vals.max() + 1)):
        ...
        keep_idxs = [
            ...
            if len(eval(row["out_list"])) == 0 and not row["edge_cell"] and not row["is_pair"]
        ]
        df_prev = df_prev.loc[keep_idxs].copy()

        # exclude current node if it already has an edge in
        df_curr = df_edges.query(
            f"{col_index_sequence} >= @idx_s and {col_index_sequence} <= @idx_s + {window}"
        )
        keep_idxs = [
            idx
            for idx, row in df_curr.iterrows()
            if len(eval(row["in_list"])) == 0 and not row["edge_cell"]
        ]
        df_curr = df_curr.loc[keep_idxs].copy()

        # redo edge mapping and splitting for edge nodes
        df_curr_has_pair = df_edges.query(
            f"{col_index_sequence} >= @idx_s and {col_index_sequence} <= @idx_s + {window}"
        )
